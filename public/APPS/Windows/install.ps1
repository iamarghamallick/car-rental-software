...
Write-Host "Installing WeDrive..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "WeDrive.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "WeDrive"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named WeDrive. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching WeDrive..."
