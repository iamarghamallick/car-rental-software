...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        WeDrive.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        WeDrive.webView.setNeedsLayout()
    }
    
    func initWebView() {
        WeDrive.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(WeDrive.webView);
        
        WeDrive.webView.uiDelegate = self;
        
        WeDrive.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            WeDrive.webView.scrollView.addSubview(refreshControl)
            WeDrive.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = WeDrive.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = WeDrive.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !WeDrive.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        WeDrive.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            WeDrive.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                WeDrive.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(WeDrive.webView.estimatedProgress);
    ...
    deinit {
        WeDrive.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: WeDrive.webView)
        }
